...
class SearchEngine:
    ...
    def clean_text(self, text: Document) -> Tuple:
        ...
        table: dict[int, int | None] = str.maketrans('', '', string.punctuation)
    ...
    def __get_website_text(self, url):
        ...
        try:
            ...
            pages = bs_transformer.transform_documents(docs, tags_to_extract=["p", "h1", "h2", "h3", "h4", "h5", "h6"])
    ...
    def retrieve_data(self) -> List[Dict[str, str]]:
        search = BingSearchAPIWrapper(
            bing_subscription_key=self.schema.origin.get('bing', {}).get('subscription-key', ''),
            bing_search_url=self.schema.origin.get('bing', {}).get('endpoint', '')
        )
        ...
        for key, value in website_data.items():
            upload_data.append({'id': str(uuid.uuid4()), 'website': key, 'content': ' '.join(value)})
